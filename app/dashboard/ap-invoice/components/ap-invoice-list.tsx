...
import { deleteAPInvoice, getAPInvoices, updateAPInvoiceStatus } from '@/lib/actions/ap-invoice'

interface APInvoicesListProps {
  apInvoices: Awaited<ReturnType<typeof getAPInvoices>>
}

export function APInvoicesList({ apInvoices }: APInvoicesListProps) {
  ...
  const handleStatusUpdate = async (id: number, status: DocStatus) => {
    setIsUpdating(id)
    try {
      const result = await updateAPInvoiceStatus(id, status)
      if (result.success) {
        toast.success('AP invoice status updated successfully')
      } else {
        toast.error(result.error)
      }
    } catch (error) {
      toast.error(`Failed to update AP invoice status ${error}`)
    } finally {
      setIsUpdating(null)
    }
  }
  ...
  const handleDelete = async (id: number, docNum: number) => {
    if (!confirm(`Are you sure you want to delete AP Invoice PCH-${docNum}?`)) return

    setIsDeleting(id)
    try {
      const result = await deleteAPInvoice(id)
      if (result.success) {
      ...
      toast.error(`Failed to delete AP invoice ${error}`)
    } finally {
      setIsDeleting(null)
    }
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <Input
            placeholder="Search AP invoices..."
            value={searchTerm}
            ...
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <CardTitle className="text-lg">PCH-{invoice.docNum}</CardTitle>
