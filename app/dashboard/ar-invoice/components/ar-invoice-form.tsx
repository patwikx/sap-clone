...
import { arInvoiceSchema } from '@/lib/validations'
import { ARInvoiceFormData } from '@/lib/types'
import { createARInvoice } from '@/lib/actions/ar-invoice'
...
export function ARInvoiceForm({ children }: ARInvoiceFormProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setBusinessPartners(partners.filter(p => p.cardType === 'C'))
      setItems(itemsList)
    }
    if (open) {
      loadData()
  ...
  const onSubmit = async (data: ARInvoiceFormData) => {
    setIsSubmitting(true)
    try {
      const result = await createARInvoice(data)
      if (result.success) {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-end space-x-2">
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? 'Creating...' : 'Create AR Invoice'}
              </Button>
            </div>
          </form>
        </Form>
