...
import { apInvoiceSchema } from '@/lib/validations'
import { APInvoiceFormData } from '@/lib/types'
import { createAPInvoice } from '@/lib/actions/ap-invoice'
...
export function APInvoiceForm({ children }: APInvoiceFormProps) {
  ...
  const onSubmit = async (data: APInvoiceFormData) => {
    setIsSubmitting(true)
    try {
      const result = await createAPInvoice(data)
      if (result.success) {
      ...
      toast.error(`Failed to create AP invoice ${error}`)
    } finally {
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="flex justify-end space-x-2">
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? 'Creating...' : 'Create AP Invoice'}
              </Button>
            </div>
          </form>
        </Form>
