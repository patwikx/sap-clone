...
import { SalesOrderWithRelations } from '@/lib/types'
...
import { deleteSalesOrder, updateSalesOrderStatus } from '@/lib/sales-orders'

interface SalesOrdersListProps {
  salesOrders: SalesOrderWithRelations[]
}

export function SalesOrdersList({ salesOrders }: SalesOrdersListProps) {
  ...
  const handleStatusUpdate = async (id: number, status: 'O' | 'C' | 'L') => {
    setIsUpdating(id)
    try {
      ...
      toast.error(`Failed to update sales order status ${error}`)
    } finally {
      setIsUpdating(null)
    }
  }
  ...
  const handleDelete = async (id: number, docNum: number) => {
    if (!confirm(`Are you sure you want to delete Sales Order SO-${docNum}?`)) return
    ...
    try {
      ...
      toast.error(`Failed to delete sales order ${error}`)
    } finally {
      setIsDeleting(null)
    }
  }
            ...
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <CardTitle className="text-lg">SO-{order.docNum}</CardTitle>
                ...
                {order.lines.length > 0 && (
                  <div className="pt-2 border-t">
                    ...
                    <div className="space-y-1">
                      {order.lines.slice(0, 3).map((line: typeof order.lines[number]) => (
